...
fn to_str(c: &mut Criterion) {
    ...
    let ruci_go = ruci::gui::Go {
        search_moves: Some(UciMoves(vec![UciMove::from_ascii(b"e2e4").unwrap(), UciMove::from_ascii(b"g2g4").unwrap()])),
        ponder: true,
        white_time: None,
        black_time: None,
    ...
    let shakmaty_uci_go = shakmaty_uci::UciMessage::Go {
        ...
        search_control: Some(shakmaty_uci::UciSearchControl {
            search_moves: vec![UciMove::from_ascii(b"e2e4").unwrap(), UciMove::from_ascii(b"g2g4").unwrap()],
            mate: Some(0),
            depth: Some(20),
            nodes: Some(57457),
        }),
    ...
    let vampirc_go = vampirc_uci::UciMessage::Go {
        ...
        search_control: Some(vampirc_uci::UciSearchControl {
            search_moves: vec![vampirc_uci::UciMove {
    ...
    group.bench_function("shakmaty_uci_to_str", |b| b.iter(|| black_box(shakmaty_uci_go.to_string())));
    group.bench_function("vampirc_uci_to_str", |b| b.iter(|| black_box(vampirc_go.to_string())));
}
...
fn from_str(c: &mut Criterion) {
    ...
    println!("ruci back to string: {}", ruci::Message::from_str(str).unwrap());
    println!("shakmaty-uci back to string: {}", shakmaty_uci::UciMessage::from_str(str).unwrap());
    println!("vampirc-uci back to string: {}", vampirc_uci::parse(str).first().unwrap());
    
    group.bench_function("ruci_from_str", |b| b.iter(|| black_box(ruci::Message::from_str(str).unwrap())));
    group.bench_function("shakmaty_uci_from_str", |b| b.iter(|| black_box(shakmaty_uci::UciMessage::from_str(str).unwrap())));
    group.bench_function("vampirc_uci_from_str", |b| b.iter(|| {
        // CLIPPY: Intentional!
